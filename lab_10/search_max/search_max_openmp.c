...
#pragma omp task final( level>max_level ) default(none) firstprivate(A,p,r,q1,level) shared(a_max_1)
    {
      if( omp_in_final() )
        a_max_1 = search_max(A,p,q1);
      else
       a_max_1 = bin_search_max_task(A,p,q1,level);
    }

#pragma omp task final( level>max_level ) default(none) firstprivate(A,p,r,q1,level) shared(a_max_2)
    {
      if( omp_in_final() )
        a_max_2 = search_max(A,q1+1,r);
      else
